...
if [ ! -d lora_gateway ]; then
    git clone https://github.com/admemoriamgrandpa/lora_gateway.git
    pushd lora_gateway
else
    pushd lora_gateway
    git pull
...
if [ "$REMOTE_CONFIG" = true ] ; then
    ...
    echo -e "{\n\t\"gateway_conf\": {\n\t\t\"gateway_ID\": \"$GATEWAY_EUI\",\n\t\t\"servers\": [ { \"server_address\": \"ec2-35-166-186-106.us-west-2.compute.amazonaws.com\", \"serv_port_up\": 1700, \"serv_port_down\": 1700, \"serv_enabled\": true } ],\n\t\t\"ref_latitude\": $GATEWAY_LAT,\n\t\t\"ref_longitude\": $GATEWAY_LON,\n\t\t\"ref_altitude\": $GATEWAY_ALT,\n\t\t\"contact_email\": \"$GATEWAY_EMAIL\",\n\t\t\"description\": \"$GATEWAY_NAME\" \n\t}\n}" >$LOCAL_CONFIG_FILE
fi
...
cp ./DTU-fotonik-gateway.service /lib/systemd/system/
systemctl enable DTU-fotonik-gateway.service
